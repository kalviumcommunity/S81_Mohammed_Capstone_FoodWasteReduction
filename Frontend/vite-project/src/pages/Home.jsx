...
function Home() {
  ...
  return (
    <div
      ...
      style={{ backgroundImage: `url('https://media.istockphoto.com/id/1145640968/photo/eco-bag-with-zero-waste-shopping-fruit-vegetables-cereals.jpg?s=612x612&w=0&k=20&c=fbTrF256ZJGLdhmEH7qECCuDdkro3Zcuc7DM9abzRfs=')` }}
    >
      <div className="bg-white/80  flex-grow py-8 px-4 md:px-10">
        {/* Welcome Banner */}
        ...
        <div className="grid gap-6">
          {groceries.length === 0 ? (
            ...
            groceries.map((item) => (
              <div
                key={item._id}
                className="bg-white/90 rounded-lg shadow-lg p-6 border border-gray-200 hover:shadow-xl transition duration-300"
              >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-gray-700">
                  ...
                  <div>
                    <p>
                      ...
                      <span className="text-red-500 font-medium"> {item.expiryDate ? new Date(item.expiryDate).toLocaleDateString() : 'N/A'}</span>
                    </p>
                  </div>
                  <div className="sm:col-span-2">
                    <p><span className="font-semibold">Storage Tip:</span> {item.storageTips || 'No tips available.'}</p>
                  </div>
                </div>
              </div>
            ))
